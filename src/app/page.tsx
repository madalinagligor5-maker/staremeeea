...
export default function HomePage() {
  return <main><PublicHeader /><InstallPrompt />
    <section className="overflow-hidden border-b hairline">
      <div className="container-shell grid min-h-[calc(100dvh-82px)] items-center gap-10 py-12 lg:grid-cols-[.88fr_1.12fr] lg:py-16">
        <div className="relative z-10 max-w-xl">
          <p className="eyebrow mb-6">Un ritm care te cuprinde</p>
          <h1 className="display text-[clamp(3.6rem,7vw,6.9rem)] font-semibold leading-[.82] tracking-[-.055em] text-[var(--wine)]">Un spațiu<br />pentru mintea ta,<br /><em className="inline-block pb-2 font-medium text-[var(--rose)]">exact așa cum este.</em></h1>
          <p className="mt-7 max-w-[55ch] text-base leading-8 text-[var(--muted)]">Organizare, emoții, focus și ritualuri de wellbeing create pentru femei cu ADHD și pentru toate cele care vor o viață mai echilibrată, cu mai multă blândețe față de ele.</p>
          <div className="mt-8 flex flex-wrap items-center gap-4"><Link className="button-primary" href="/inregistrare">Începe acum <ArrowRight size={17} /></Link><span className="text-sm text-[var(--muted)]">Poți începe gratuit. Fără card.</span></div>
          <p className="display mt-9 rotate-[-2deg] text-2xl italic text-[var(--rose-dark)]">Pași mici. Schimbări care rămân.</p>
        </div>
    ...
    <section aria-label="Ce găsești în Starea Mea" className="border-b hairline bg-white/50 py-8">
    ...
    <section className="py-24 sm:py-32" id="instrumente">
      <div className="container-shell">
        <div className="grid items-end gap-6 md:grid-cols-[1fr_.72fr]"><div><p className="eyebrow">Instrumente pentru ziua ta</p><h2 className="display mt-4 max-w-3xl text-5xl font-semibold leading-[.95] tracking-[-.04em] text-[var(--wine)] sm:text-7xl">Tot ce ai nevoie,<br /><em className="font-medium text-[var(--rose)]">într-un singur loc.</em></h2></div><div><p className="leading-7 text-[var(--muted)]">Instrumente simple și intuitive, create special pentru tine. Folosește-le direct în browser sau instalează Starea Mea pe telefon.</p><Link href="/instrumente" className="mt-5 inline-flex items-center gap-2 text-sm font-bold text-[var(--rose-dark)]">Explorează instrumentele <ArrowRight size={16} /></Link></div></div>
    ...
    <section className="paper-noise py-24 text-center sm:py-32"><div className="container-shell"><p className="display text-3xl italic text-[var(--rose)]">Tu contezi.</p><h2 className="display mx-auto mt-4 max-w-4xl text-5xl font-semibold leading-none text-[var(--wine)] sm:text-7xl">Începe astăzi să ai mai multă grijă de tine.</h2><p className="mt-6 text-[var(--muted)]">Este timpul pentru Starea Mea.</p><Link className="button-primary mt-8" href="/inregistrare">Intră în spațiul tău <ArrowRight size={17} /></Link></div></section>
    <PublicFooter />
  </main>;
}
